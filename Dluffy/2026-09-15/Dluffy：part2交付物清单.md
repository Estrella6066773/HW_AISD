...
> 口径：本清单由全组共同圈定范围、主笔汇总成稿。Due Date 按 workshop 节奏——A / B 两类为本周三（计划增量）定稿；C 类实现排到 28 Sep 的 Sprint。
...
|-------------|------------------------|--------------|----------|---------------------|
| 案例理解与问题总结 | 案例 8 问（背景 / 利益相关者 / 主流程 / 问题 / 规则 / 异常 / 外部系统 / 信息） | Part 1（A、C） | 周三 | 8 问逐项有答案；推断项标注为假设 |
...
| 需求追溯矩阵 | 需求 ↔ 模型 ↔ 实现 ↔ 测试 | Part 3 / Part 4（B、D） | 28 Sep | 每行可回溯到 Backlog ID |
| 验收标准 | 按工作包 | Part 4（A） | 周三 | 每条可测、有对应证据 |

## B. Agile 过程证据

| Deliverable | Related Requirement(s) | Dependencies | Due Date | Acceptance Criteria |
|-------------|------------------------|--------------|----------|---------------------|
| Product Backlog | Part 3 | B 主笔 | 周三 | 12 字段齐全（ID / 名称 / 描述 / 优先级 / 关联需求 / 估算 / 一责 / 二责 / 依赖 / 验收 / 证据 / 状态） |
| Definition of Done | 全组约定 | C 提议 | 周三 | 含证据最低要求（Backlog ID + 文件/commit + 截图） |
| Sprint Backlog + Goal | Part 6 | C 主笔 | 周三 | Sprint 1 目标 + 条目带双负责人与验收 |
| Standup 记录与汇总 | Part 7 | 每人自写、D 汇总 | 周三起每人 ≥ 1 次 | 含完成 / 证据 / 下一步 / 阻塞 / 支持 / 交接 |
| Sprint Review | Part 8 | A 主笔 | 周三 | 展示分工、完成 / 未完成、证据、反馈 |
| 反馈与 Backlog 变更 | Part 8 | D 维护 | 持续 | 变更可追溯 |
...
| 个人工作区与分工文 | 协作规则 | 按日期建文件夹 | 持续 | 四人在各自文件夹（Ruby / Est / Dluffy / Ryan）上传 |
...
- **周三硬交付（计划增量）**：A 类（分析与模型）+ B 类（Agile 过程证据）。
- **28 Sep 交付（实现增量）**：C 类（实现与验证）。
- D 类为全程持续项。
