...
**状态**：步骤 2 已规定变量名。概念图未改。两个网关条件已写入可执行副本。人工填写的变量由表单进入流程，见 [`Est/est：治疗预约与付款表单.md`](est：治疗预约与付款表单.md)。外部工作器如何写入 `payment_status`、`transaction_reference`、`payment_date` 和 `confirmation_sent`，见 [`Est/est：治疗预约与付款外部工作器.md`](est：治疗预约与付款外部工作器.md)。Java 工程尚未创建。
