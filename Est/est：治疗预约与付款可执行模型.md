...
**状态**：步骤 3 已在个人目录放好可执行副本。概念图未改。表单文件尚未创建，Java 程序尚未编写。
...
组内目录 `docs/operational-bpmn/` 先不放入这份副本。用户任务上的表单编号还只是占位，等步骤 4 做出表单后再放入，避免组目录里出现还打不开的表单。
...
六个用户任务都指定处理人 `demo`，并挂上表单编号。步骤 4 按这些编号创建表单后再替换内容。当前目录里还没有这些表单文件。
...
Camunda Modeler 打开这份副本时，可以对齐任务类型、消息名、关联键和网关条件。在表单文件还不存在时，任务列表里还不能提交这些表单。
